...
<pre><code>
<script id="my-template" data-jstemplate="ref" type="text/html">
 ...
</script>
</code></pre>
...
<pre><code>
 <a onclick="`{ user.save }()">Click me</a>
</code></pre>

This is very useful for exposing values which exist during template evaluation as values
that can be used in the generated JS code itself.
...
arrive later via AJAX). When you pass the @future@ tag a value which is an instance of the type @JSTemplates.Future@, the current template tag evaluation is delayed until the _future_ instance is defined
...
<pre><code>
~{future value:new JSTemplates.Future("/Application/data"), var:'elements' }
 <ul>
...
Note that you can define your own subclasses of @JSTemplate.Future@ to define new types of ways
to get to asynchronous data.
...
If you pass a single unnamed parameter it will be named @_arg@ but you can also
explicitely name them (in which case they should be accessed with a leading underscore)
and even pass in a body to be expanded with the @doBody@ tag:
